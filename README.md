...
| `opencode-login` | OpenCode credentials with **OAuth as the default**: `/login opencode` (+ `opencode-go`, which adopts the Zen account) runs the `opencode console login` device flow; `/login` still lets you pick "Sign in with an API key". `/opencode` shows status/OAuth guidance, `/opencode set <key>` explicitly mirrors one API key to Zen + Go (a bare `/opencode <key>` is refused), plus `copy`/`status` |
